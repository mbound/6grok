...
impl GsmtapSink {
    ...
    ///
    /// `packet` begins with the DIAG command byte (for example `0x10` for a
    /// LOG_F record). HDLC escaping, CRC and the `0x7e` delimiter are not
    /// included because GSMTAP encapsulates the DIAG payload itself.
...
///
/// Fields not meaningful for the selected payload are zero. Multi-byte fields
/// are network byte order as required by GSMTAP.
