...
//
// Qualcomm DIAG framing/control primitives shared conceptually with the
// original 6grok agent implementation. The log-config packet layout is also
// independently corroborated by P1sec/QCSuper at commit
// aa555b4f7f25f7a8bf4e5afd4dcb884edf2f6735.
...
pub fn set_mask_request(
    ...
    for &item in items {
        ...
        mask[usize::from(item) / 8] |= 1 << (item % 8);
    }
...
pub fn parse_log_config_header(
    packet: &[u8],
) -> Result<(LogConfigHeader, &[u8]), ControlError> {
...
mod tests {
    ...
    #[test]
    fn builds_qcsuper_compatible_log_config_header() {
        assert_eq!(retrieve_id_ranges_request(), vec![0x73, 0, 0, 0, 1, 0, 0, 0]);
